...
public class ConvertLead extends SvrProcess {
	...
	protected String doIt() throws Exception {
		...
		bp.set_ValueOfColumn("DS_OrgMaster_ID", lead.get_Value("DS_OrgMaster_ID"));
		bp.set_TrxName(get_TrxName());
		if (lead.getBPName()!=null)
			bpName = lead.getBPName();
		else
			bpName = lead.getName();
		...
		int count = DB.getSQLValue(get_TrxName(), "Select count(*) From C_BPartner_Location Where C_Location_ID=?",lead.getC_Location_ID());
		if (lead.getC_Location_ID() != 0 && count<=0 )
		{
		...
		count = DB.getSQLValue(get_TrxName(), "Select count(*) From C_BPartner_Location Where C_Location_ID=?",lead.getBP_Location_ID());
		// company address
		if (lead.getBP_Location_ID() != 0 && count<=0)
		{
