	...
	protected void prepare()
	{
		...
		for (int i = 0; i < para.length; i++)
		{
			...
			if (para[i].getParameter() == null)
				...
				p_C_OrderSource_ID = para[i].getParameterAsInt();
			else
...
//			sql.append("IsSOTrx='N'");
		if(p_IsSOTrx)
		{
			...
			if(p_C_OrderSource_ID>0)
				sql.append(" and i.C_Order_ID IN (SELECT c.C_Order_ID FROM C_Order c WHERE c.IsSoTrx='Y' and c.C_OrderSource_ID="+p_C_OrderSource_ID+") ");
		}
		else
