 ...
 * 
 */
public class DSInvoiceGen extends GenForm
{
	/**	Logger			*/
	private static CLogger log = CLogger.getCLogger(DSInvoiceGen.class);
	//
	
	public Object 			m_AD_Org_ID = null;
	public Object 			m_C_BPartner_ID = null;
	public Object 			m_C_orderType_ID = null;
	public boolean			m_DontShowZeroValue = false;
	public boolean			m_ShowConsignmentInvoices = false;
	public String			m_SalesOrderDocNo = "";
	...
	private String getOrderSQL()
	{
        ...
        if (m_C_orderType_ID != null)
            sql.append(" AND ic.C_DocType_ID=").append(m_C_orderType_ID);
        if(m_DontShowZeroValue)
        	sql.append(" AND ic.totallines <> 0 ");
        if(m_ShowConsignmentInvoices)
        	sql.append(" AND bp.SendEMail = 'Y' ");
        else
        	sql.append(" AND bp.SendEMail = 'N' ");
        
        if(!m_SalesOrderDocNo.isEmpty())
        	sql.append(" AND ic.DocumentNo like '%").append(m_SalesOrderDocNo).append("%' ");
