...
ValueChangeListener,Serializable,WTableModelListener  
{
	...
	private void fillPicks() throws Exception 
	{
		...
		int PP_Order_ID=0;
		String fromProcess = Env.getContext(ctx, "DS_FromProcess");
		if(fromProcess.equals("true"))
		{
			PP_Order_ID = Env.getContextAsInt(Env.getCtx(), "1|0|PP_Order_ID");
		}
		...
		orderField.setValue(PP_Order_ID);
...
//		locatorField = new WLocatorEditor(MLocator.COLUMNNAME_M_Locator_ID, true, false, true, locatorL, m_WindowNo);
		...
		MLookup MLocatorLookup = MLookupFactory.get(ctx, m_WindowNo, 0,
			  	   MColumn.getColumn_ID(MLocator.Table_Name, MLocator.COLUMNNAME_M_Locator_ID),
			  	   DisplayType.TableDir);
		locatorField = new WSearchEditor("M_Locator_ID", false, false, true, MLocatorLookup);
		...
		//  Switch Tabs
			...
			Messagebox.show(Msg.getMsg(Env.getCtx(), "Update"), "", Messagebox.OK | Messagebox.CANCEL, Messagebox.QUESTION, new org.zkoss.zk.ui.event.EventListener() 
			{
			    public void onEvent(Event evt) throws InterruptedException {
			        ...
			        {
			        	System.out.println(isOnlyReceipt());
			        	if(isOnlyReceipt())
			        	{
...
/*			int result = -1;			
			...
			if ( result == Messagebox.OK)
				Messagebox.show("Ok !","Info", Messagebox.OK, Messagebox.INFORMATION);
			else 
				Messagebox.show("CANCEL !","Info", Messagebox.OK, Messagebox.INFORMATION);
//			if ( result == 1)
/*			{				
	...
	private void setLocator() 
	{
		if(m_PP_order!=null)
		{
			MProduct product = new MProduct(Env.getCtx(), m_PP_order.getM_Product_ID(), null);
			setM_Locator_ID(product.getM_Locator_ID());
		}
	...
	public void valueChange(ValueChangeEvent e)
	{
		...
		if (name.equals("PP_Order_ID"))
		{
			...
			if (pp_order != null)
			{
				...
				pickcombo.setSelectedIndex(2);  //default to first entry - isBackflush
				if(pickcombo.getSelectedIndex()==2)
					setLocator();
