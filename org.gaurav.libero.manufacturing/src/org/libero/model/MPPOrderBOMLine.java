...
public class MPPOrderBOMLine extends X_PP_Order_BOMLine
{
			...
			String trxName)
	{
		...
		MProduct product = new MProduct(p_ctx, bomLine.getM_Product_ID(), get_TrxName());
		...
		else
		{
			...
			BigDecimal requiredQty = order.getQtyEntered().multiply(bomLine.getQty());
			int AttributeSetInstance = getAvailableBatchNumber(bomLine,bomLine.getM_Product_ID(),bomLine.getM_AttributeSetInstance_ID(),
					requiredQty,getPP_Order_BOMLine_ID());
			setM_AttributeSetInstance_ID(AttributeSetInstance);
		}
	...
	public int getAvailableBatchNumber(MPPProductBOMLine bomLine, int M_Product_ID, int M_Locator_ID,
			BigDecimal qtyEntered, int orderBOMLineID) 
	{
		int attributeSet = 0;
		BigDecimal qtyRequired = qtyEntered;
		String sql = "SELECT M_AttributeSetInstance_ID,sum(qtyonhand) FROM M_StorageOnHand "
				+ "WHERE M_Product_ID=? "
				+ " AND M_AttributeSetInstance_ID > 0 and m_locator_id in (select loc.m_locator_id from m_warehouse wh , m_locator loc where wh.m_warehouse_id=loc.m_warehouse_id and Wh.Isintransit<>'Y') "
				+ " group by M_AttributeSetInstance_ID "
				+ "having sum(qtyonhand)<>0 ORDER BY M_AttributeSetInstance_ID";
		...
		try {
			...
			if (rs.next()) 
			{
				int FirstASI = rs.getInt("M_AttributeSetInstance_ID");

				int locatorID = getLocator(M_Product_ID, attributeSet);
				MLocator loc = new MLocator(getCtx(), locatorID, get_TrxName());
				loc.getM_Warehouse_ID();
				
				BigDecimal qtyAvailable = MStorageReservation.getQtyAvailable(loc.getM_Warehouse_ID(), M_Product_ID, attributeSet, get_TrxName());
				System.out.println();
				
				if (qtyAvailable.compareTo(qtyRequired) >= 0) 
				{
					attributeSet = FirstASI;
				} 
			}
	...
	private int getLocator(int m_Product_ID, int attributeSet) {

		PreparedStatement pstmt;
		ResultSet rs = null;
		int mLocatorID = 0;
		String sql = "select loc.m_locator_id from m_storageonhand st,m_locator loc,m_warehouse war where"
				+ " st.m_locator_id=loc.m_locator_id and loc.m_warehouse_id=war.m_warehouse_id "
				+ "and st.m_product_id=? and st.m_attributesetinstance_id=? and war.IsInTransit<>'Y' and st.QtyOnHand > 0 ";
		pstmt = DB.prepareStatement(sql, null);
		try {
			pstmt.setInt(1, m_Product_ID);
			pstmt.setInt(2, attributeSet);
			rs = pstmt.executeQuery();
			if (rs.next()) {
	...
	protected void reserveStock()
	{
		...
		/*
		 * Code change by Gaurav Sontakke 
		 * Posted in idempiere google group
		 * Correction in the method of MStorageReservation.
		 * 
		 */
