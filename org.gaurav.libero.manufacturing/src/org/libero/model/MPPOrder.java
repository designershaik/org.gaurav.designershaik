...
public class MPPOrder extends X_PP_Order implements DocAction
{
	...
	private void calculateCosting(MPPOrderBOMLine[] lines) 
	{
		for(MPPOrderBOMLine line:lines)
		{
			CorrectCost(line.getM_Product_ID());
		}
		
	}

	private void CorrectCost(int M_Product_ID) 
	{
		MProduct product=new MProduct(getCtx(), M_Product_ID, get_TrxName());
		if(product.isBOM())
		{
			MPPOrderBOMLine[] bom=getBOMComponents(product);
		}
	 ...
	 * @return Order BOM Lines
	 */
	public MPPOrderBOMLine[] getBOMComponents(MProduct product)
	{
		String whereClause = MPPProductBOMLine.COLUMNNAME_M_Product_ID+"=?";
		List<MPPOrderBOMLine> list = new Query(getCtx(), MPPProductBOMLine.Table_Name, whereClause, get_TrxName())
										.setParameters(new Object[]{getPP_Order_ID()})
										.setOrderBy(MPPOrderBOMLine.COLUMNNAME_Line)
										.list();
		m_lines = list.toArray(new MPPOrderBOMLine[list.size()]);
	...
	private void orderStock()
	{
		...
		//	Update Storage
			if (!MStorageReservation.add(getCtx(), getM_Warehouse_ID(),
					getM_Product_ID(), getM_AttributeSetInstance_ID(), ordered, false, get_TrxName()))
			{
...
//	@Override
	...
	private void expandBOM(final MPPProductBOM PP_Product_BOM, MPPOrderBOM PP_Order_BOM, BigDecimal parentQty) {
		...
		{				
			...
			if (PP_Product_BOMline.isValidFromTo(getDateStartSchedule()))
			{
				...
				/* Code change by Gaurav Sontakke 
				 * 20 June 2016 . 
				 * Added option to switch between creating automated work order & requisition 
				 * 
				 */
				
				boolean IscreateRequisition=MSysConfig.getBooleanValue("GS_ORDER_CREATE_REQUISITION", false);
				boolean IsCreateWOForBOM=MSysConfig.getBooleanValue("GS_CREATE_WO_FORBOM_ITEM", false);
				...
				{
					createRequisitionIFpurchased(product, obl.getQtyRequired());
				}
	...
	private static boolean IscomponentIssued(MPPOrder order) 
	{
		boolean issued = true;
		MPPOrderBOMLine[] bomlines = order.getLines();
		for(MPPOrderBOMLine line : bomlines)
		{
			if(line.getQtyReserved().intValue()>0)
			{
				issued = false;
				throw new AdempiereException("DS_MO_ComponentsNotIssued");
			}
		}
		return issued;
	}
