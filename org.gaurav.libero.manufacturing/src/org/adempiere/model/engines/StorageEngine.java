...
public class StorageEngine 
{
	...
	{	
		...
		if (product != null && product.isStocked())
		{
			...
			Timestamp dateMPolicy= MovementDate;
			
			Timestamp t = MStorageOnHand.getDateMaterialPolicy(docLine.getM_Product_ID(), docLine.getM_AttributeSetInstance_ID(), docLine.get_TrxName());
			if (t != null)
				dateMPolicy = t;
			...
			else
			{
				...
				if (!MStorageOnHand.add(docLine.getCtx(), M_Warehouse_ID, 
					docLine.getM_Locator_ID(),
					docLine.getM_Product_ID(), 
					docLine.getM_AttributeSetInstance_ID(), Qty,dateMPolicy, docLine.get_TrxName()))
				{
					throw new AdempiereException(); //Cannot correct Inventory (MA)
				}
